...
def filter_grib_data(file_path, country_geojson):
    logger.info("Filtering GFS data from %s.", file_path)
    country_shape = shape(geojson.loads(country_geojson))
    filtered_data = []

    try:
        gribs = pygrib.open(file_path)
        for grib in gribs:
            param_name = grib.parameterName.lower().replace(' ', '_')
...
def parse_and_import_gfs_data(file_path, relevant_parameters, country, base_time):
    ...
    country_geojson = country.geom.geojson
    filtered_data = filter_grib_data(file_path, country_geojson)
    forecast_data = []

    valid_date = base_time + timedelta(hours=forecast_hour - 1)

    for data in filtered_data:
        param_key = data['param_name']
    ...
    def handle(self, *args, **kwargs):
        ...
        for country in countries:
            try:
                ...
                grib_files = download_gfs_data_sequence(base_url, date, hours_to_try, forecast_hours, "data/gfs_files")
                if not grib_files:
